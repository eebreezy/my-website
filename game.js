(() => {
  ...
  // Prevent page scroll when interacting with the game area on mobile
  ...
  // ===== Responsive canvas sizing (important for mobile/pc) =====
  ...
  // Input: tap/click anywhere on canvas OR jump button
  ...
  function drawPlayer(){
    ...
    ctx.lineWidth=3; ctx.strokeStyle="#071023";
    ctx.beginPath(); ctx.arc(x,y+4,7,0.1*Math.PI,0.9*Math.PI); ctx.stroke();
  ...
  function drawStar(s,t){
    ...
    const x=s.x, y=s.y+wob;

    ctx.globalAlpha=0.18;
    ctx.fillStyle="#ffd66e";
    ctx.beginPath(); ctx.arc(x,y,s.r*1.9,0,Math.PI*2); ctx.fill();
    ctx.globalAlpha=1;
    ...
    ctx.beginPath(); ctx.arc(x+4,y-1,1.5,0,Math.PI*2); ctx.fill();
    ctx.lineWidth=2; ctx.strokeStyle="#071023";
  ...
  function drawCloud(c,t){
    ...
    ctx.beginPath(); ctx.ellipse(x,y,w*0.5,h*0.4,0,0,Math.PI*2); ctx.fill();
    ...
    ctx.lineWidth=2;
    ctx.beginPath(); ctx.ellipse(x,y,w*0.5,h*0.4,0,0,Math.PI*2); ctx.stroke();
    ctx.globalAlpha=1;
  ...
  function update(dt){
    ...
    if (player.y > bottomBound - player.r){ player.y = bottomBound - player.r; player.vy = 0; }

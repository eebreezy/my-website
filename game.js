(() => {
  ...
  // ===== Simple sound synth (no external files) =====
  ...
  function beep({ freq = 440, dur = 0.08, type = "sine", gain = 0.05, sweep = 0 }) {
    ...
    if (sweep) osc.frequency.exponentialRampToValueAtTime(freq * sweep, t0 + dur);
  ...
  const state = {
    ...
    level: 1,
    nextLevelAt: 120,
    mood: 0, // unlocks visuals
  };

  elBest.textContent = state.best;

  // World units in CSS pixels (we draw in CSS space because ctx transform handles DPR)
  ...
  btnOverlay.addEventListener("click", () => { ensureAudio(); startGame(); });
  ...
  function togglePause() {
    ...
    if (state.paused) showOverlay("Paused", "Tap Play to resume.", "Resume");
    else hideOverlay();
  }
  ...
  function drawPlayer() {
    // Cute blob character with eyes + blush
    const x = player.x;
    const y = player.y;
    const r = player.r;
    ...
    if (!blink) {
      ...
      ctx.beginPath(); ctx.arc(x - 8.2, y - 5.2, 1.2, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(x + 5.8, y - 5.2, 1.2, 0, Math.PI * 2); ctx.fill();
    } else {
      ctx.lineWidth = 3;
    ...
    ctx.beginPath(); ctx.arc(x - 12, y + 4, 4.5, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(x + 12, y + 4, 4.5, 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha = 1;
  }
  ...
  function tick(now) {
    ...
    // If canvas size changed because of layout, keep DPR fit
  ...
  function update(dt) {
    ...
    if (player.y < topBound + player.r) { player.y = topBound + player.r; player.vy = 0; }
    if (player.y > bottomBound - player.r) {
      player.y = bottomBound - player.r;
      player.vy = 0;
    }

    // spawn pacing (keeps it addictive but not overwhelming)
    ...
    // collisions: clouds (obstacles)
    for (let i = clouds.length - 1; i >= 0; i--) {
      const c = clouds[i];
      const cy = c.y + Math.sin(state.time * 2 + c.puff) * 4;
      const rx = c.x - c.w * 0.5, ry = cy - c.h * 0.45, rw = c.w, rh = c.h * 0.9;
      ...
      if (rectCircleHit(rx, ry, rw, rh, player.x, player.y, player.r)) {
        ...
        // penalty: break streak & score drop (but not too punishing)
        state.streak = 0;
        state.score = Math.max(0, state.score - 25);
        syncHUD();

        // if too many hits in a row, end game (keeps sessions from dragging)
        // Here: end if score is low and level is low, otherwise keep going.
    ...
    // slow score drip rewards time played (keeps kids feeling progress)
    if (Math.floor(state.time * 2) % 2 === 0) {
      // very small, not every frame
      // (we'll do it using a tiny accumulator)
    }
  ...
  function render(dt) {
    ...
    ctx.fillText("Tap / Click to jump • Space works too", 16, 24);
    ctx.globalAlpha = 1;
  }

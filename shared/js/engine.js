...
(function(){
  ...
  function fit(){
    ...
    const wrap = $('#three');
    const w = Math.max(1, Math.round((vv && vv.width) || wrap.clientWidth || window.innerWidth));
    const h = Math.max(1, Math.round((vv && vv.height) || wrap.clientHeight || window.innerHeight));
    const dpr = Math.min(window.devicePixelRatio || 1, (w * h > 1100000 ? 1.15 : (w < 900 ? 1.25 : 1.5)));
  ...
  function fallbackPlay(note, bankName, strong){
    ...
    if(type === 'drum'){
      ...
      gain.gain.linearRampToValueAtTime(strong ? 0.32 : 0.20, now + 0.008);
      ...
      ng.gain.linearRampToValueAtTime(bankName === 'woodblock' ? 0.03 : 0.08, now + 0.004);
    ...
    gain.gain.cancelScheduledValues(now);
    gain.gain.setValueAtTime(0.0001, now);
    gain.gain.linearRampToValueAtTime(strong ? 0.13 : 0.09, now + 0.012);
    gain.gain.setTargetAtTime(0.0001, now + 0.09, bankName === 'organ' || bankName === 'choir' ? 0.42 : 0.24);
  ...
  async function accent(i, strong){
    ...
    if(accentSynth && strong && (clicks % 5 === 0)){
      try{
        accentSynth.volume.value = -30;
        accentSynth.triggerAttackRelease(noteAt(i), '64n');
        return;
      }catch(e){}
    }
  }
